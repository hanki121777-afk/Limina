# Agent Workflow: Limina 시스템 제어 가이드 (agent.md)

> 이 문서는 Limina의 **시스템 작동 규칙**을 정의합니다.
...
Limina은 유저의 PC 사용 중 발생하는 텍스트 데이터를 초경량으로 수집하고,
...
**저장 위치**:
- Windows: `%APPDATA%/Limina/daily_log.txt`
- Mac: `~/Library/Application Support/Limina/daily_log.txt`
- Linux: `~/.config/Limina/daily_log.txt`
...
총 28개 태그. 선택값은 `config.json`의 `user.interest_categories` 필드에 저장.
마이페이지 "아이디어 설정" 섹션에서 언제든 수정 가능.

### Step 4 — 사업화 조건 설정 ★ 신규

유저의 현실적 조건을 설정. AI가 아이디어 채점 시 최대한 반영.
(100% 보장은 아니지만 맞는 방향으로 기울어짐)

| 설정 항목 | 옵션 | 기본값 |
|---|---|---|
| 초기 예산 | ~100만원 / ~500만원 / ~2,000만원 / 제한없음 | ~500만원 |
| 투자 가능 시간 | 주말만 / 퇴근 후 1~2시간 / 반일 / 풀타임 | 퇴근 후 1~2시간 |
| 선호 형태 | 혼자 / 팀(2~3명) / 상관없음 | 혼자 |
| 온/오프라인 | 온라인만 / 오프라인 OK / 상관없음 | 온라인만 |
| 수익 목표 | 부업 월 50만원 / 부업 월 200만원 / 풀타임 대체 / 스케일업 | 부업 월 200만원 |

선택값은 `config.json`의 `user.business_conditions` 필드에 저장.
마이페이지 "사업화 조건" 섹션에서 언제든 수정 가능.
...
**즉시 트리거 키워드 분석 → v2로 이동 (MVP 미포함)**
- MVP에서는 Idle 타임 기반 분석만 운영
- 트리거 키워드 기능은 v2에서 Pro 전용으로 추가 예정
...
### 6-4. 동적 밸런서 (Dynamic Balancer) — 황금선 유지

퀄리티와 빈도 사이의 **황금 밸런스**를 자동으로 유지하는 핵심 시스템.

> **핵심 철학:**
> "매일 숙제 검사하듯 별로인 아이디어를 억지로 쥐어짜서 보여주면 유저는 실망하고 앱을 삭제한다.
> 진짜 제대로 된 '노다지 아이디어'가 걸려든 그 순간에만 팝업을 띄워야 유저가 다음을 기대한다."

| 컨트롤 | 작동 방식 | 효과 |
|---|---|---|
| **최대 침묵 기한** | 5일 이상 알림 없으면 REPORT 강제 출력 | "프로그램 고장" 오인 방지 |
| **커트라인 자동 하향** | 5일 침묵 시 퀄리티 모드 임시 완화 | 방치감 해소 |
| **등급별 톤앤매너** | GOLD(흥분), SILVER(가벼움), BRONZE(트렌디), REPORT(정보) | 유저 기대치 관리 |
| **밀당 알림 카피** | 내용 안 보여주고 궁금증 유발 | 클릭률 극대화 |
| **신규 유저 임계치 완화** | 첫 사용 시 1일치 데이터로도 분석 | 첫 경험 빠르게 제공 |

### 6-5. 최대 침묵 기한 (Max Silence Cap)
...
**핵심 원칙 — 밀당 알림 배달:**
> 유저가 "이거 뭐야?" 하고 클릭하게 만드는 게 목표.
> 아이디어 내용을 팝업에 다 보여주면 안 됨. 궁금증을 유발하고 대시보드로 끌어당겨야 함.

| 등급 | 한국어 | English | 日本語 |
|---|---|---|---|
| GOLD | 💡 지난 N일 분석 결과, 특급 아이템 1개 포착 | 💡 After N days — Premium idea found | 💡 N日分析完了、特級アイデア発見 |
| SILVER | 가벼운 비즈니스 영감이 도착했습니다 | A light business spark arrived | 軽いビジネスアイデアが届きました |
| BRONZE | 트렌드 아이디어 1개 발견 | Trend-based idea spotted | トレンドアイデアを1件発見 |
| REPORT | 이번 주 관심사 키워드 정리됐습니다 | Your weekly interest summary is ready | 今週の関心キーワードをまとめました |

**GOLD 팝업 카피 예시 (한국어):**
```
💡 특급 아이디어 포착

지난 4일간의 수집 데이터 분석 결과...
드디어 실현 가능성 높은 특급 아이템 1개를 포착했습니다.

[확인하기 →]
```

**GOLD 팝업 카피 예시 (English):**
```
💡 Premium Idea Detected

After analyzing 4 days of your activity...
We've found 1 high-potential business idea worth your attention.
...
```typescript
// 브라우저 기본 앱으로 대시보드 열기
shell.openExternal(`https://Limina.com/dashboard/ideas/${ideaId}?lang=${userConfig.language}`);
```
...
온도계, 실행 난이도, 유사 사례 검색, 시장 분석, 사업계획 구체화, 게시판/공유 등은 **Limina이 제공하지 않음.**

유저가 프롬프트를 복사해서 본인이 쓰는 AI(ChatGPT, Claude, Gemini 등)에게 직접 물어보는 방식으로 처리.

> **Limina은 "아이디어 생성 + 전달"까지만 책임진다. 그 이상은 유저의 AI 영역.**
...
### 페이월 전략 — GOLD 잠금이 핵심 전환 트리거

Free 유저는 SILVER 알림 몇 번 받다가 자연스럽게 Pro로 전환된다.
전환 흐름:

```
Free 유저가 SILVER 알림 수신
→ "꽤 괜찮네?" 체험
→ "특급 아이디어가 있다는데 GOLD는 잠겨있음"
→ "한 달에 9,900원이면..." 고민
→ Pro 전환
```

**GOLD 잠금 UI 처리:**
- 대시보드에서 GOLD 카드는 블러 처리 + 자물쇠 아이콘
- "Pro로 업그레이드하면 볼 수 있습니다" 메시지
- 알림 자체는 오지만 내용은 가림 → 궁금증 극대화

### 얼리버드 전략 (베타 출시용)

**목표:** 베타 출시 시 얼리버드 300명 판매
**방법:** 정기결제 대비 40% 할인 연간권

| | 한국 | 미국 | 일본 |
|---|---|---|---|
| 얼리버드 연간권 | 69,000원 | $49.99 | ¥4,980 |
| 정기결제 12개월 | 118,800원 | $95.88 | ¥11,760 |
| 할인율 | 42% | 48% | 58% |

**한국 기준 얼리버드 목표:**
```
300명 × 69,000원 = 20,700,000원 초기 자금
→ 서버비 + API 비용 약 1년치 버퍼 확보
→ v2 개발 자금
```

**얼리버드 심리 트리거:**
- "지금 사면 평생 이 가격" 강조 (가격 올릴 예정 공지)
- 얼리버드 배지 (대시보드에 표시)
- 남은 얼리버드 자리 카운트다운
...
> **"떴다 하면 유저가 고개를 끄덕일 만한 퀄리티"** × **"스팸처럼 느껴지지 않을 만큼 적당한 빈도"**
> = 제품 수명을 10배 늘리는 동적 밸런서 시스템
