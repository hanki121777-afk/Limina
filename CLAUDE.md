...
예시:
> "Supabase 대시보드 좌측 메뉴에서 'Authentication'을 클릭하세요. 그 다음 상단 탭 'Providers'를 누르면 'Email' 카드가 보입니다. 우측 토글을 켜고 'Save' 버튼을 누르세요."
...
- 없거나 v18 이하면 설치 필요:
  ...
  - **Windows**:
    ...
    2. winget이 안 되면 ("winget을 찾을 수 없습니다" 에러), [nodejs.org](https://nodejs.org)에서 LTS 버전 다운로드 안내
    3. 설치 완료 후, **반드시 터미널을 완전히 껐다가 다시 열기**
    4. 새 터미널에서 `node -v`로 확인. 여전히 안 되면 컴퓨터를 재시작
...
mkdir chrome-extension     # 크롬/Edge 익스텐션
mkdir web-dashboard        # Next.js 웹 대시보드
mkdir landing-page         # Next.js 랜딩페이지
mkdir shared              # 공통 타입/유틸 (TypeScript)
```
...
#### Windows 사용자 사전 점검
위 단계 시작 전에 아래를 먼저 점검하세요:

- **터미널 종류**: PowerShell을 사용하세요. cmd(명령 프롬프트)는 쓰지 마세요. 사용자가 cmd를 쓰고 있으면 PowerShell로 바꾸라고 안내하세요.
  - PowerShell 여는 방법: Windows 키 → "PowerShell" 입력 → 클릭
- **PowerShell 실행 정책 차단**: npm 첫 실행 시 "스크립트를 실행할 수 없습니다" 에러가 나면 다음을 먼저 실행:
  ```powershell
  Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
  ```
- **`npm` 명령이 안 될 때**: "npm은(는) 내부 또는 외부 명령이 아닙니다"라고 나오면, Node.js 설치 후 터미널을 껐다 열지 않은 것입니다. **터미널을 완전히 닫고 새로 열기**.

---

### 9. 팝업창 및 중요 동작 버튼 확인 규칙

- **커스텀 모달 의무화**: 시스템 기본 알림창(`alert`, `confirm`, `window.prompt`)은 절대 사용하지 마세요. 알림이나 확인창이 필요할 때는 반드시 Framer Motion 애니메이션이 가미된 **커스텀 모달**로 구현하세요.
- **중요 동작의 즉각 실행 금지**: 데이터 삭제, 구독 취소, 계정 삭제 같은 **중대한 동작은 클릭 시 즉시 실행되면 안 됩니다.** 실수를 방지하기 위해 반드시 실행 전에 **확인 모달**을 거치도록 강제하세요.
...
**마케팅 표현 가이드:**
- ✅ "당신의 일상 데이터는 PC를 떠나지 않습니다. AI가 만든 아이디어만 안전하게 보관됩니다."
- ❌ "모든 데이터가 로컬에만 저장됩니다" (이건 부정확)

---

### 12. 카카오톡/메신저 처리 (법적 안전)

**모든 메신저 앱의 메시지를 직접 수집하는 행위는 금지됩니다.** (법적 리스크)

- ❌ 카카오톡 PC 앱 직접 읽기
- ❌ Slack/Discord/LINE/WhatsApp/Telegram API 후킹
- ❌ Notion/Obsidian 데이터베이스 직접 접근

대신 **클립보드 모니터링**으로 통일:
- 유저가 메신저에서 텍스트를 **직접 복사(Ctrl+C)** 했을 때만 수집
- "클립보드 변경 감지 → 익명화 → daily_log 저장"

이 규칙은 글로벌 전체에 동일 적용됩니다.
...
- **API 키 보관**:
  - 데스크톱 앱: OS 자격증명 관리자 (Win: `keytar`, Mac: Keychain, Linux: libsecret)
  - 웹: Vercel 환경변수 (서버 사이드)
  - 절대 평문 저장 금지
...
- 예: "지금은 크롬 익스텐션에서 YouTube 페이지 텍스트를 수집하는 모듈을 짜는 단계예요."
...
1. **CLAUDE.md / GEMINI.md** (이 파일) — 최상위 절대 규칙
...
- 환경변수 접두사:
  ...
  - 웹 대시보드/랜딩: `NEXT_PUBLIC_` (Next.js 클라이언트), 그 외는 서버 전용
- 개발 서버 주소:
  - 데스크톱 앱 내부 HTTP 서버: `localhost:7421` (익스텐션 수신용)
  - 웹 대시보드: `localhost:3000`
  - 랜딩페이지: `localhost:3001`
- **npm 명령어를 체인하지 마세요** (`&&`로 연결 금지). 한 줄씩 실행하고 결과를 확인한 뒤 다음으로 넘어가세요.
- 배포는 `npx vercel --prod`를 사용하세요. `npm install -g vercel`은 사용하지 마세요.
...
### "winget을 찾을 수 없습니다"
winget이 설치 안 된 컴퓨터예요. [nodejs.org](https://nodejs.org)에 접속해서 LTS 버전 다운로드 버튼을 클릭하고, 설치 파일을 실행하라고 안내하세요. 옵션은 전부 기본값으로 "Next"만 누르면 됩니다.
...
1. PowerShell을 오른쪽 클릭 → "관리자 권한으로 실행"
2. `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser` 입력
3. "Y" 입력 후 Enter
4. PowerShell 닫고 다시 일반 사용자 권한으로 열기

### Electron 빌드 실패
- 첫 빌드는 시간이 오래 걸려요 (5~10분). 기다리세요.
- 안티바이러스(특히 Windows Defender)가 `node_modules\electron\` 접근을 차단할 수 있어요. 일시 비활성화 또는 폴더 예외 추가.
...
Homebrew 미설치. 다음 명령으로 설치 안내:
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```

### Mac: Electron 앱 "확인되지 않은 개발자" 경고
- 개발 중에는: 우클릭 → 열기 → 다시 열기
- 배포 시에는 코드 사이닝 + 공증(notarization) 필요 (`dev.md` 참조)
